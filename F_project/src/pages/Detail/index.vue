        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 与之前的路由跳转相比，此处在跳转之前，发送请求将购买的产品信息发送给服务器 -->
...
export default {
  ...
  methods: {
    ...
    // 加入购物车的回调函数
    async addShopCart() {
      // 1.给服务器发送请求，将产品加入到数据库中
      // 2.服务器存储成功————进行路由跳转并传递参数
      // 3.若失败，给用户进行提示
      // 下面这行代码其实就是调用仓库中的addOrUpdateShopCart函数，而此函数加上了async，则返回的一定是Promise对象
      try {
        await this.$store.dispatch("addOrUpdateShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        // 路由跳转并传递产品参数(skuNum参数由于较为简单，可以以query的形式传递，而skuInfo较复杂，可以以会话存储的形式传递，会话结束立即消失)
